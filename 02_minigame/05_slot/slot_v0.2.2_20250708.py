...
def get_frame_positions_and_facing():
    ...
    if 45 <= yaw < 135:      # West
        ...
        positions = [(x-2, y+1, z-1), (x-2, y+1, z), (x-2, y+1, z+1)]
    elif 135 <= yaw < 225:   # North
        facing = 3  # south
        positions = [(x-1, y+1, z-2), (x, y+1, z-2), (x+1, y+1, z-2)]
...
def roll_slot(index, positions, facing):
    global rolling, results, stop_flag
    idx = 0
...
# === Update subtitle ===
def update_subtitle():
    # Extract ore names only and join with spaces to show in subtitle
    names = []
    for b in results:
        if b and ":" in b:
    ...
    with m.EventQueue() as eq:
        ...
        while idx < 3 and not stop_flag:
            ...
            if event.type == m.EventType.KEY and event.action == 0 and event.key == 257:
                rolling[idx] = False
                m.echo(f"✅ Slot {idx+1} stopped: {results[idx]}")
                update_subtitle()
                idx += 1
