...
def build_furnace_system(offset_x, color, shulker_list, player_name):
    ...
    m.execute(f"setblock {base_x} {base_y} {base_z} minecraft:{color}_shulker_box[facing=north]")
...
def main():
    ...
    if arg == "set2":
        ...
        for entry in data:
            ...
            m.execute(
                f'setblock {entry["x"]} {entry["y"]+1} {entry["z"]} '
        ...
        m.echo("Player heads + big heads placed.")
