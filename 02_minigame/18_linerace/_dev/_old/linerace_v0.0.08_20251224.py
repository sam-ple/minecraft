    ...
    for i, p in enumerate(cfg["players"]):
        ...
        for d in range(cfg["length"]):
            ...
            for y in range(sy + SEARCH_UP, sy - SEARCH_DOWN, -1):
                b = m.getblock(x, y, z)
                if b in GROUND_BLOCKS:
                    m.execute(f"setblock {x} {y} {z} {block}")
                    break

    save_lanes(lanes)
    chat("🟦 LineRace setup complete (lanes generated)", "aqua")
...
# ==============================
def start_game():
    global game_active, game_start_time, lanes, max_dist
