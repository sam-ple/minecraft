...
def cmd_start():
    ...
    # 弓
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run clear @s minecraft:bow'
    )
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run give @s minecraft:bow[enchantments={"minecraft:infinity":1},unbreakable={}] 1'
    )

    # 矢
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run clear @s minecraft:arrow'
    )
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run give @s minecraft:arrow 1'
    )

    # 釣り竿
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run clear @s minecraft:fishing_rod'
    )
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run give @s minecraft:fishing_rod[enchantments={"minecraft:luck_of_the_sea":3,"minecraft:lure":3,"minecraft:unbreaking":3,"minecraft:mending":1}] 1'
    )

    # 最後にタグ付与（ここ重要）
    m.execute(
        'execute as @a[tag=!started] at @s if block ~ ~-1 ~ minecraft:gold_block run tag @s add started'
    )
