...
def listen_chat_commands():
    global got, lap_times, prev_items, last_status_time, is_running
    with m.EventQueue() as eq:
        ...
        while True:
            ...
            if event.type == m.EventType.CHAT:
                ...
                if "!start" in msg and not is_running:
                    start_game()
                    ...
                    m.execute(f"bossbar remove {bossbar_id}")
                    print_separator()
                    tellraw_message([("Game has been reset.", "red")])
                    print_separator()

                elif "!check" in msg and is_running and now - last_status_time > STATUS_COOLDOWN:
