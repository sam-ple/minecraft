...
def bossbar_init():
    global bossbar_initialized
    if not bossbar_initialized:
        ...
        bossbar_initialized = True

def bossbar_set(name, progress):
    """
    progress: 0.0 〜 1.0 の残り時間割合
    """
    bossbar_init()
    ...
    m.execute(f"bossbar set {BOSSBAR} color {color}")
...
def countdown():
    for i in [3, 2, 1]:
        title_subtitle(str(i), "Get ready!", "red", "white")
        # カウントダウン中もボスバー表示＆色変化
        progress = i / 3  # 3→1 カウントダウンなので 1.0〜0.33
        bossbar_set(f"Starting in {i}s...", progress)
        time.sleep(1)
    title_subtitle("Start!", "", "green")
    bossbar_set("Start!", 1.0)
    time.sleep(1)
    bossbar_clear()
            ...
            for item_id in current_inv:
                if item_id in POINTS:
                    ...
                    if curr_count > prev_count:
                        ...
                        title_subtitle(f"+{gained} pts", item_id.split(":")[1], "aqua", "white")
                        m.execute(f'title @a actionbar {{"text":"Score: {score}","color":"gold"}}')
