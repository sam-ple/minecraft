...
最初に進捗を取った人に追加で1pt
最初の人の名前をハイライト
end_game() のリザルト表示に「ボーナス獲得リスト」を追加して、誰が FIRST を取ったか一覧表示
"""
...
def start_game():
    ...
    global last_remaining, last_name, current_start_pos, first_advancements
    
    first_advancements = set()  # ←ゲーム開始ごとにリセット
    last_remaining = None
    last_name = None
...
def end_game():
    ...
    # --- Winner Celebration ---
    if sorted_players:
        winner, pts = sorted_players[0]

        # Title & subtitle
        m.execute(f'title @a title {{"text":"🏆 Winner: {winner}","color":"gold","bold":true}}')
        m.execute(f'title @a subtitle {{"text":"{pts} pts","color":"yellow","bold":true}}')

        # Firework celebration (spawn fireworks around the winner)
        for _ in range(5):
            m.execute(f'execute at {winner} run summon firework_rocket ~ ~1 ~ {{LifeTime:40,FireworksItem:{{id:"minecraft:firework_rocket",Count:1,tag:{{Fireworks:{{Flight:1,Explosions:[{{Type:1,Colors:[I;16711680,65280,255],FadeColors:[I;16776960]}}]}}}}}}}}')
    ...
    if event and event.type == EventType.CHAT:
        ...
        if game_active and match:
            ...
            if advancement_name not in first_advancements:
                ...
                add_points += 1
                m_chat(f"✨ {player_name} is the FIRST to achieve '{advancement_name}'! (+1 bonus)")

            # ポイント加算
            player_points[player_name] = player_points.get(player_name, 0) + add_points
            ...
            # 通知
            m_echo(f"{player_name} earned {add_points} point(s) for '{advancement_name}'! (Total: {player_points[player_name]})")
            m.execute('playsound minecraft:block.note_block.chime master @a')

            # ログ保存
            save_adv_text(player_name, advancement_name)
            continue  # ← ここで他の処理に流れないようにしてOK
