...
while True:
    ...
    if event and event.type == EventType.CHAT:
        ...
        if msg.startswith("<") and ">" in msg:
            ...
            # allowed_players = ["crocadooo", "friend1", "friend2"]
            # if player_name not in allowed_players:
            #     continue

            # ゲーム中かどうかでコマンド制限
            if game_active and not (content.startswith("--adv") or content == "--stop"):
                m.echo("Game in progress! Only --adv and --stop are available.")
                continue
            ...
            # --settime <秒>
                ...
                if len(parts) == 2 and parts[1].isdigit():
                    ...
                    m.echo(f"Game duration set to {GAME_DURATION} seconds.")
                else:
                    m.echo("Usage: --settime <seconds>")

            # --setpos <x>_<y>_<z>
            elif content.startswith("--setpos"):
                parts = content.split()
                if len(parts) == 2 and re.match(r"^\d+_\d+_\d+$", parts[1]):
                    START_POS = parts[1].replace("_"," ")
                    m.echo(f"Start position set to {START_POS}")
                else:
                    m.echo("Usage: --setpos <x>_<y>_<z>")
