...
def m_chat(msg: str):
    """全員に通知"""
    m.execute(f'tellraw @a {json.dumps({"text": msg, "color": "red", "bold": True})}')
...
def end_game():
    ...
    for i, (player, pts) in enumerate(sorted_players):
        ...
        msg = {"text": f"{rank}: {player} ({pts} pts)", "color": color, "bold": bold}
    ...
    if event and event.type == EventType.CHAT:
        ...
        if msg.startswith("<") and ">" in msg:
            ...
            if content.startswith("--"):
                if game_active and not (content.startswith("--adv") or content in ["--stop", "--help","--status"]):
                    m_chat("Game in progress! Only --adv, --help and --stop are available.")
                    continue
                ...
                if content == "--start" and not game_active:
                    ...
                    if game_active:
                        ...
                        m_chat(f"Game in progress! Remaining time: {remaining} seconds")
                    else:
                        m_chat(f"No game running. Default duration: {GAME_DURATION} seconds")
                    ...
                    if len(parts) == 2:
                        ...
                        for line in lines:
                            if line.startswith(target + ":"):
                                found = True
                                m_chat(f"{player_name} requested --adv: {line.strip()}")
                        if not found:
                            m_chat(f"{player_name} requested --adv: No advancements found for {target}")
                elif content == "--help":
                    help_texts = [
