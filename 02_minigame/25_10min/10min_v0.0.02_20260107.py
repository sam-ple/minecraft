...
def title_sub(msg, color="yellow"):
    m.execute(
        f'title @a subtitle {json.dumps({"text": msg, "color": color})}'
    )
...
def clear_effect(elapsed):
    ...
    # for _ in range(5):
    #     m.execute(
    #         'execute at @s run summon firework_rocket ~ ~1 ~ '
    #         '{LifeTime:40,FireworksItem:{id:"minecraft:firework_rocket",Count:1,tag:{Fireworks:{'
    #         'Explosions:[{'
    #         'Type:1,'
    #         'Colors:[I;16711680,65280,255],'
    #         'FadeColors:[I;16776960],'
    #         'Trail:1b,'
    #         'Flicker:1b'
    #         '}]'
    #         '}}}}'
...
def start_game(mode):
    ...
    if mode == "adv":
        ...
        m.execute('scoreboard objectives add progress dummy "Advancements"')
        m.execute("scoreboard objectives setdisplay sidebar progress")
...
while True:
    ...
    if event and event.type == EventType.CHAT and game_active:
        ...
        if CURRENT_MODE == "adv":
            adv_count += 1
            m.execute(f"scoreboard players set @a progress {adv_count}")
