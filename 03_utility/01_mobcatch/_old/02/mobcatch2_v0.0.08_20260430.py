...
# Scoreboard
# ==============================
m.execute("scoreboard objectives remove used")
m.execute("scoreboard objectives remove used_prev")
...
while True:
    ...
    for name in PLAYERS:
        ...
        # m.execute(
        #     f'execute if entity @e[tag={tag}] run tag @e[tag={tag}] remove {tag}'
        # )
        ...
        #     f'if entity @s[nbt={{SelectedItem:{{id:"{ITEM}"}}}}] '
        #     f'at @s anchored eyes positioned ^ ^ ^{DIST} '
        #     f'if entity @e[type=!player,type=!armor_stand,distance=..2] '
        #     f'run tag @e[type=!player,type=!armor_stand,limit=1,sort=nearest,distance=..2] add {tag}'
        # )
        m.execute(
            f'execute as @a[name={name}] '
            f'if entity @s[nbt={{SelectedItem:{{id:"{ITEM}"}}}}] '
            f'at @s anchored eyes positioned ^ ^ ^{DIST} '
            f'if entity @e[type=!player,type=!armor_stand,type=!item,type=!item_frame,type=!minecart,distance=..2] '
            f'run tag @e[type=!player,type=!armor_stand,type=!item,type=!item_frame,type=!minecart,limit=1,sort=nearest,distance=..2] add {tag}'
        )

        # 光る（距離無関係）
        # m.execute(f"effect give @e[tag={tag}] glowing 1 1 true")
        m.execute(
            f'execute if entity @e[tag={tag}] run effect give @e[tag={tag}] glowing 1 1 true'
        )

        # パーティクル
        m.execute(
            f'execute if entity @e[tag={tag}] '
            f'at @e[tag={tag}] run particle minecraft:happy_villager ~ ~1 ~ 0.2 0.2 0.2 0 3'
        )

    # ==========================
    # ② クリック検知
    # ==========================
    # m.execute('execute as @a run tag @s remove just_clicked')
    m.execute('execute if entity @a run tag @a remove just_clicked')
    # m.execute('execute as @a if score @s used > @s used_prev run tag @s add just_clicked')
    m.execute('execute if entity @a as @a if score @s used > @s used_prev run tag @s add just_clicked')
    ...
    for name in PLAYERS:
        ...
        m.execute(
            ...
            f'if entity @e[tag={tag},limit=1] '
            f'run tag @s add has_target'
        )

        # いる時だけ処理
        for mob, egg in SPAWN_EGG_MAP.items():
            m.execute(
                f'execute as @a[name={name},tag=just_clicked,tag=has_target] '
                f'if entity @e[tag={tag},type={mob},limit=1] '
                f'run give @s {egg} 1'
            )

            mob_name = mob.replace("minecraft:", "")

            m.execute(
                f'execute as @a[name={name},tag=just_clicked,tag=has_target] '
                f'if entity @e[tag={tag},type={mob},limit=1] '
                f'run tellraw @a ['
                f'{{"selector":"@s","color":"gold"}},'
                f'{{"text":" caught {mob_name}","color":"green"}}'
                f']'
            )
        ...
        # 後処理
        # m.execute('tag @a[tag=has_target] remove has_target')
        m.execute('execute if entity @a[tag=has_target] run tag @a[tag=has_target] remove has_target')

    # ==========================
    # ④ 後処理
    # ==========================
    # m.execute("execute as @a run scoreboard players operation @s used_prev = @s used")
    m.execute("execute if entity @a as @a run scoreboard players operation @s used_prev = @s used")
