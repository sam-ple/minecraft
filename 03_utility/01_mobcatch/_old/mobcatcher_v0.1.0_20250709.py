    ...
    with EventQueue() as eq:
        ...
        while True:
            ...
            if event.type == EventType.KEY and event.action == 1 and event.key in (257, 335):
                ...
                # 上空テレポート＆NoAI付与で固定
                try:
                    # Y座標256にテレポート（ワールドによっては高さ変更可能）
                    m.execute(f"data merge entity {target.uuid} {{NoAI:1b}}")
                    m.execute(f"tp {target.uuid} ~ 256 ~")
                except:
                    try:
                        m.execute(f"data merge entity {target.selector} {{NoAI:1b}}")
                        m.execute(f"tp {target.selector} ~ 256 ~")
                    except:
