...
def get_key_name_code(key_code: int) -> str:
    """Return key name with code, e.g., W(87)"""
...
t_nearest_ore = Hud.add_text("", 5, next_line_y()) 
...
nearest_ore_info = "Nearest Ore: None"

def ore_scan_loop():
    global nearest_ore_info
    ...
    while True:
        try:
            ...
            for blk, (dx, dy, dz) in zip(blocks, offsets):
                ...
                # minecraft: を削除
                blk_name = blk.replace("minecraft:", "")

                if blk_name in TARGET_ORES:
            ...
            if nearest:
                blk_type, blk_pos = nearest
                nearest_ore_info = (
                    f"Nearest Ore: {blk_type} "
                    f"@ {blk_pos} ({nearest_dist:.1f}m)"
                )
            else:
                nearest_ore_info = "Nearest Ore: None"

        except Exception as e:
            nearest_ore_info = "Nearest Ore: Error"

        sleep(0.5)

threading.Thread(target=ore_scan_loop, daemon=True).start()
...
while True:
    try:
        ...
        if block:
            ...
            block_text = f"Block: {block.type.replace('minecraft:', '')} @ ({bx},{by},{bz})"
        else:
        ...
        Hud.set_text_string(t_nearest_ore, nearest_ore_info)
