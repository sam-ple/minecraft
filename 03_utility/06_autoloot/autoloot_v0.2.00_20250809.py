...
# Pull from chest (skip weak gear) → close → open inventory → upgrade to strongest armor & best sword → close
import time
...
# ---------- NEW: weak gear filter ----------
_WEAK_MATS = {"leather", "wood", "chainmail", "turtle", "stone", "gold"}  # don't pull these from chest
...
def is_weak_gear(item_id: str) -> bool:
    """Return True if the item is armor/sword in weak materials we want to skip."""
...
def equip_best_armor_and_best_sword():
    ...
    # 2) Sword: pick best in 9..44 vs hotbar0 (view 36)
    items = refresh_items()
...
def pull_then_upgrade():
    ...
    try:
        ...
        m.echo(f"✅ Pulled (no weak gear) {moved} / Upgraded {upgraded} (armor & sword)")
    finally:
        _busy = False
...
kb.set_keybind(
    ...
    name="PullAndUpgrade",
    category="Minescript+",
    description="Pull chest (skip leather/wood/chainmail/turtle gear), then auto-upgrade armor & move best sword to hotbar 0."
)

m.echo("🎹 G: Pull (skip weak leather/wood/chainmail/turtle gear) → close → open inventory → swap to stronger armor/sword → close")
while True:
    time.sleep(1)