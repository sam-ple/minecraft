...
# H: Spawn weighted-loot chest (incl. bow/arrow/buckets/stone) → G: Pull & upgrade + hotbar layout → J: Remove chest only
...
def spawn_weighted_loot_chest():
    ...
    # Utility / consumables / blocks (NEW)
    add(items, ["minecraft:bow"],            W_UNCOMMON, max_stack=1)
    add(items, ["minecraft:arrow"],          W_COMMON,   max_stack=64)
    add(items, ["minecraft:golden_apple"],   W_RARE,     max_stack=64)
    add(items, ["minecraft:water_bucket"],   W_UNCOMMON, max_stack=1)
    add(items, ["minecraft:lava_bucket"],    W_RARE,     max_stack=1)
    add(items, ["minecraft:stone"],          W_COMMON,   max_stack=64)
...
def remove_last_chest_only():
    ...
    m.echo("🗑️ Removed the spawned chest (contents were not cleared).")
...
# ---- 追加：ホットバー固定配置（剣0、弓1、矢2、右側=石8/雪玉7/水6/溶岩5/金リンゴ4）----
def _place_item_to_hotbar_exact(item_id: str, hotbar_index: int) -> bool:
    """そのアイテムを hotbar.{index}（view=36+index）へ持っていく。成功したらTrue"""
    ...
    sources = []
    for slot, st in items.items():
        if st.item == item_id and slot != 45:  # exclude offhand
            if slot != 36:
                sources.append(slot)
    # 最後の手段として36も許容（剣じゃない時だけ）
    if (36 in items) and items[36].item == item_id:
        sources.append(36)
...
def layout_hotbar_tools():
    changed = 0
    # 左側：弓→1、矢→2
    if _place_item_to_hotbar_exact("minecraft:bow", 1): changed += 1
    if _place_item_to_hotbar_exact("minecraft:arrow", 2): changed += 1
    # 右側固定：8 石, 7 雪玉, 6 水, 5 溶岩, 4 金リンゴ
    if _place_item_to_hotbar_exact("minecraft:stone", 8): changed += 1
    if _place_item_to_hotbar_exact("minecraft:snowball", 7): changed += 1
    if _place_item_to_hotbar_exact("minecraft:water_bucket", 6): changed += 1
    if _place_item_to_hotbar_exact("minecraft:lava_bucket", 5): changed += 1
    if _place_item_to_hotbar_exact("minecraft:golden_apple", 4): changed += 1
    return changed

# ---- G: Pull → close → open inventory → upgrade/equip → hotbar整列 → close ----
def pull_then_upgrade():
    ...
    try:
        ...
        m.echo(f"✅ Pulled (no weak gear) {moved} / Upgraded {upgraded} (armor & sword) / Shield: {bool(shielded)} / Hotbar laid out: {laid_out}")
    finally:
        _busy = False
...
kb.set_keybind(71, pull_then_upgrade,          name="PullAndUpgrade",   category="Minescript+", description="Pull chest (skip weak gear) → upgrade armor/sword → equip shield → layout hotbar.")  # G
kb.set_keybind(74, remove_last_chest_only,     name="RemoveLootChest",  category="Minescript+", description="Remove the last spawned chest (no content clearing).")  # J

m.echo("🎹 H: Spawn weighted-loot chest | G: Pull→upgrade→shield→layout | J: Remove spawned chest")
while True:
    time.sleep(1)